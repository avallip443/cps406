  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      reports.forEach(report => {
        ...
        bugsByStatus[report.bugStatus] = (bugsByStatus[report.bugStatus] || 0) + 1;

        // Count bugs by priority
        bugsByPriority[report.priorityLevel] = (bugsByPriority[report.priorityLevel] || 0) + 1;
      });
...
const graphData = {
    ...
    datasets: [
      {
        label: 'New Bugs',
        data: [sprintStats.sprint1.total - sprintStats.sprint1.closed, sprintStats.sprint2.total - sprintStats.sprint2.closed], // Assuming 'total' includes both 'active' and 'closed'
        borderColor: '#FA9EBC',
  ...
  if (isLoading) {
    return <Flex justify="center" align="center" height="100vh"><Text color="#0B1957">Loading...</Text></Flex>;
  }
      ...
      <Flex bgColor={'#f8f3ea'}>
        ...
        <Container maxW="container.lg">
          <VStack spacing={5} align="stretch">
            <Heading color="#0B1957">Bug Statistics</Heading>
            <Text color="#0B1957">Total Bugs: {totalBugs}</Text>
            <Box>
              <Heading size="md" color="#0B1957">Bugs by Status:</Heading>
              {Object.entries(bugsByStatus).map(([status, count]) => (
                <Text key={status} color="#0B1957">{`${status}: ${count}`}</Text>
              ))}
            </Box>
            <Box>
              <Heading size="md" color="#0B1957">Bugs by Priority Level:</Heading>
              {Object.entries(bugsByPriority).map(([level, count]) => (
                <Text key={level} color="#0B1957">{`${level}: ${count}`}</Text>
              ))}
            </Box>
            <Text color="#0B1957">Sprint 1 - Total: {sprintStats.sprint1.total}, Active: {sprintStats.sprint1.active}, Closed: {sprintStats.sprint1.closed}</Text>
            <Text color="#0B1957">Sprint 2 - Total: {sprintStats.sprint2.total}, Active: {sprintStats.sprint2.active}, Closed: {sprintStats.sprint2.closed}</Text>
            {/* Existing content above */}

{/* Add the graph component here */}
<Box>
  <Heading size="md" color="#0B1957">Bug Trends by Sprint</Heading>
  <Line data={graphData} />
</Box>
